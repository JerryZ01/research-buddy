...
7. 小标题要有信息量，能看出这一节的观点（如「为什么 I/O 密集场景不受 GIL 影响」），避免「概述」「背景介绍」这类空标题；章节按主题逻辑重组，不要机械地按子问题逐条罗列。
8. 语气专业、克制、自信；语言自然流畅，像一个有经验的人认真讲清楚一件事，避免套话、模板腔和「综上」「众所周知」式的空泛表达。
9. 论点直接陈述，**不需要**标注引用编号或来源链接（文末参考文献由系统自动生成）。
10. 如果提供了「可用插图」，在内容最相关的位置插入 1~2 张插图：`![alt文本](图片URL)`。图片 URL 必须原样来自「可用插图」列表，禁止使用列表之外的图片；alt 文本用「可用插图」里给出的描述。
...
### 文章结构（参考）
- 开篇：点出问题、为什么重要、本文将给出什么判断
- 主体：按 2~4 个有洞察的主题组织（每个主题 = 观点 + 证据 + 分析/权衡）
- 结尾：关键结论 + 实践建议 + 展望（给读者可执行的行动指引）"""
...
### 可信度与表达
7. 事实与数据必须来自检索证据，保持准确；资料有限或不确定处明确说明（如「截至本文写作时」「现有资料显示」），并区分事实、分析与推测，不做没有依据的断言。
8. 小标题要有信息量，能看出这一节的观点，避免「概述」「背景介绍」这类空标题；章节按主题逻辑重组，不要机械地按子问题逐条罗列。
9. 语气专业、克制、自信；语言自然流畅，避免套话、模板腔和「综上」「众所周知」式的空泛表达。
10. 论点直接陈述，**不需要**标注引用编号或来源链接（文末参考文献由系统自动生成）。
11. 如果提供了「可用插图」，在内容最相关的位置插入 1~2 张插图：`![alt文本](图片URL)`。图片 URL 必须原样来自「可用插图」列表，禁止使用列表之外的图片；alt 文本用「可用插图」里给出的描述。
...
### 可信度与表达
6. 事实与数据必须来自检索证据，保持准确；资料有限或不确定处明确说明（如「截至本文写作时」「现有资料显示」），并区分事实、分析与推测，不做没有依据的断言。
7. 小标题要有信息量，能看出这一节的观点，避免「概述」「背景介绍」这类空标题；章节按主题逻辑重组，不要机械地按子问题逐条罗列。
