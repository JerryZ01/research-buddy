...
10. 如果提供了「可用插图」，在内容最相关的位置插入插图（**整篇最多 4 张**）：优先插入能说明内容的图，也允许插入 1~2 张与主题氛围相符的装饰图作为点缀；每张图都应与所在内容相关或契合（根据 alt 描述判断），不要堆砌无关的图：`![alt文本](图片URL)`。图片 URL 必须原样来自「可用插图」列表，禁止使用列表之外的图片；alt 文本用「可用插图」里给出的描述。
...
11. 如果提供了「可用插图」，在内容最相关的位置插入插图（**整篇最多 4 张**）：优先插入能说明内容的图，也允许插入 1~2 张与主题氛围相符的装饰图作为点缀；每张图都应与所在内容相关或契合（根据 alt 描述判断），不要堆砌无关的图：`![alt文本](图片URL)`。图片 URL 必须原样来自「可用插图」列表，禁止使用列表之外的图片；alt 文本用「可用插图」里给出的描述。
