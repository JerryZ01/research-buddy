...
### 技术图解（涉及架构/原理/流程时必做）
13. 如果主题涉及**技术架构、系统原理、工作流程**（如微服务架构、编译流程、协议交互、算法机制），用 Mermaid 绘制图解（架构图/流程图/时序图/状态图）。**数量取决于内容需要，不设死数**：复杂架构或长流程可以在每个关键环节各画一张（3~6 张），简单原理 1~2 张即可，用文字能讲清就不画——图解的价值是让读者少读一段字，画之前自问「这张图能省下读者的理解成本吗」。格式示例：
```mermaid
graph TD
    A[用户请求] --> B[网关]
    B --> C{{鉴权}}
