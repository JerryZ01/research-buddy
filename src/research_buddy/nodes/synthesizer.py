...
11. 如果提供了「可用插图」，在内容最相关的位置插入插图（**整篇最多 {image_limit} 张、通常 4~6 张即可**，各章节合理分配）。插图按优先级：① 能说明内容的图（图表/架构图/截图）② 与主题相关的配图 ③ 氛围装饰图（全篇不超过 2 张）。每张图插入前自问：它能帮助读者理解这一段吗？不能就不插——宁可少一张，不放无关的图。：`![alt文本](图片URL)`。图片 URL 必须原样来自「可用插图」列表，禁止使用列表之外的图片；alt 文本用「可用插图」里给出的描述。
