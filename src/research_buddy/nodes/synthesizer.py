...
10. 如果提供了「可用插图」，在内容最相关的位置插入插图（**整篇最多 3 张**）：只有图片内容确实能说明/支撑相邻论述时才插入——根据 alt 描述判断图文是否匹配，不匹配就不插，宁可无图也不要放不相关的图：`![alt文本](图片URL)`。图片 URL 必须原样来自「可用插图」列表，禁止使用列表之外的图片；alt 文本用「可用插图」里给出的描述。
...
11. 如果提供了「可用插图」，在内容最相关的位置插入插图（**整篇最多 3 张**）：只有图片内容确实能说明/支撑相邻论述时才插入——根据 alt 描述判断图文是否匹配，不匹配就不插，宁可无图也不要放不相关的图：`![alt文本](图片URL)`。图片 URL 必须原样来自「可用插图」列表，禁止使用列表之外的图片；alt 文本用「可用插图」里给出的描述。
